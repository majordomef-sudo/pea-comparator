...
(function () {
  ...
  const SCORE_MAP = (function(){var m={};if(window.ETF_SCORES){for(var i=0;i<window.ETF_SCORES.length;i++){m[window.ETF_SCORES[i].isin]={s:window.ETF_SCORES[i].total,d:window.ETF_SCORES[i].decision};}}return m;})();
  function scoreColor(v){if(v>=85)return "#28a745";if(v>=70)return "#007bff";if(v>=55)return "#ffc107";if(v>=45)return "#fd7e14";return "#dc3545";}
  ...
  function sortData(col, asc) {
    state.filtered.sort((a, b) => {
      ...
      if (col === 'frais' || col === 'perf5') {
        const na = (va === '' || va === 'N/A' || va === null || va === undefined) ? (col === 'frais' ? 999 : -999) : parseNum(va);
  ...
  function render() {
    ...
    if (pageData.length === 0) {
      ...
      tbody.innerHTML = pageData.map(e => {
        const fv = e.frais !== '' && e.frais !== 'N/A' ? parseNum(e.frais) : NaN;
        const frais = !isNaN(fv) ? fv.toFixed(2) + '%' : '—';
        const sri = e.sri || '—';
        const pv5 = e.perf5 !== '' && e.perf5 !== 'N/A' ? parseNum(e.perf5) : NaN;
        const perf5 = !isNaN(pv5) ? pv5.toFixed(1) + '%' : '—';
        let perfClass = 'perf-neutral';
        if (!isNaN(pv5)) {
          if (pv5 > 0) perfClass = 'perf-positive';
          else if (pv5 < 0) perfClass = 'perf-negative';
        }
        const fraisClass = !isNaN(fv) ? 'frais-vert' : '';
        ...
        return `<tr data-isin="${e.isin}">
          ...
          <td>${e.emetteur || '—'}</td>
          <td>${e.peap ? '<span class="badge-pea">PEA</span>' : '—'}</td>
          <td class="score-cell" style="text-align:center;font-weight:bold">${SCORE_MAP[e.isin] ? '<span style="color:'+scoreColor(SCORE_MAP[e.isin].s)+'" title="'+SCORE_MAP[e.isin].d+'">'+SCORE_MAP[e.isin].s+'/100</span>' : '—'}</td>
  ...
  // === INIT ===
  if (!ROWS.length && window.PEA_ETFS && window.PEA_ETFS.length) { ROWS = window.PEA_ETFS; }
  state.filtered = [...ROWS];
  render();
...
(function() {
  ...
  const ROWS = window.PEA_ETFS || [];
  const DATA_UPDATE_DATE = '15/09/2026';
  ...
  if (etfBody) {
    etfBody.addEventListener('click', function(e) {
...
(function() {
  ...
  function addScoreColumn() {
    var tbody = document.querySelector('#etfBody');
    if (!tbody) return;
...
// === Audit 2 — compteur ETF dynamique === 
(function(){try{var n=(window.PEA_ETFS||[]).length;if(n){document.querySelectorAll('[data-etf-count]').forEach(function(el){el.textContent=n;});}}catch(e){}})();